...
m_blast_db=/media/sequentia/sharing/src/006-ncRNA/datasets/Generic/Uniprot_SwissProt_0421.fasta
prot_db=/media/sequentia/sharing/src/006-ncRNA/datasets/Generic/Uniprot_SwissProt_0421.fasta
...
test -x blastall || (echo "Can't find blastall on your path, eheck it!" > /dev/stderr && exit 1)
...
#if test ! -h "${m_blast_db}.phr" -a ! -h "${m_blast_db}.00.phr"; then
#    m_blast_db=$DATA_DIR/uniref90_82
#    if test ! -h "${m_blast_db}.phr"  -a ! -h "${m_blast_db}.00.phr"; then
...
if [[ $nostranded == "OK" ]]; then
	echo -e "CPC - BLASTX - both strands for unstranded transcripts..."
	blast_opts="-S 3"
else
	echo -e "CPC - BLASTX - one strand for stranded transcripts..."
...
blast_opts="$blast_opts -a 16";     # 2 CPUs, boost the performance
blast_opts="$blast_opts -d $m_blast_db"	# database settings

# Framefinder settings
if [[ $nostranded == "OK" ]]; then
	ff_opts="-r True -w $m_framefinder_model /dev/stdin"
	echo -e "CPC - FrameFinder - both strands for unstranded transcripts..."
else
	ff_opts="-r False -w $m_framefinder_model /dev/stdin"            # only the same strand
	echo -e "CPC - FrameFinder - one strand for stranded transcripts..."
fi
