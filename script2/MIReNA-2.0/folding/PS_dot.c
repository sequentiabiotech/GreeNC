...
static char UNUSED rcsid[] =
  "$Id: PS_dot.c,v 1.34 2006/01/18 12:58:42 ivo Exp $";
...
PUBLIC int PS_rna_plot_a (char *string, char *structure, char *ssfile,
			  char *pre, char *post);
PUBLIC int PS_rna_plot (char *string, char *structure, char *ssfile, char *name, int begin, int end);
...
PUBLIC int
gmlRNA (char *string, char *structure, char *ssfile, char option)
{
  FILE *gmlfile;
  int i;
  int length;
  ...
  switch (option)
	...
	i = naview_xy_coordinates (pair_table, X, Y);
      ...
      if (i != length)
	fprintf (stderr, "strange things happening in gmlRNA ...\n");
      break;
    default:
  ...
  fprintf (gmlfile,
	   ...
	   "# CreationDate: %s\n"
	   "# Name: %s\n"
	   "# Options: %s\n", VERSION, time_stamp (), ssfile,
	   option_string ());
  fprintf (gmlfile, "graph [\n" " directed 0\n");
  for (i = 1; i <= length; i++)
    {
      fprintf (gmlfile, " node [ id %d ", i);
      if (option)
	fprintf (gmlfile, "label \"%c\"", string[i - 1]);
  ...
  for (i = 1; i < length; i++)
    fprintf (gmlfile, "edge [ source %d target %d ]\n", i, i + 1);
  for (i = 1; i <= length; i++)
    {
      if (pair_table[i] > i)
	fprintf (gmlfile, "edge [ source %d target %d ]\n", i, pair_table[i]);
...
/*---------------------------------------------------------------------------*/
int
PS_rna_plot (char *string, char *structure, char *ssfile, char *name, int begin, int end)
{
	char *pre1= "/leftpos xmin xmax add 2 div size 2 div sub def\n/toppos ymin ymax add 2 div size 2 div add def\n/HelveticaBoldItalic .5 size 350 div add LabelFont\nleftpos size 100 div add toppos size 100 div 2 mul sub moveto\n(";
	char *pre2= ") show\n0 0 0 setrgbcolor\n/HelveticaBold .8 LabelFont";
	char *pre= malloc ((strlen (pre1) + strlen (name) + strlen (pre2) + 1) * sizeof (char));
	strcat (pre, pre1);
	strcat (pre, name);
	strcat (pre, pre2);

	char *post= malloc (23 * sizeof (char));
	sprintf (post, "%d %d 2 1 0 0 omark\n", begin, end);

  return PS_rna_plot_a (string, structure, ssfile, pre, post);
}
...
int
PS_rna_plot_a (char *string, char *structure, char *ssfile, char *pre,
	       char *post)
{
  ...
  if (rna_plot_type == 0)
    ...
    i = naview_xy_coordinates (pair_table, X, Y);
  if (i != length)
    fprintf (stderr, "strange things happening in PS_rna_plot...\n");
...
int
svg_rna_plot (char *string, char *structure, char *ssfile)
{
  float xmin, xmax, ymin, ymax, size;
  int i, length;
  float *X, *Y;
  ...
  fprintf (xyplot,
	   ...
	   SIZE / size, SIZE / size, (size - xmin - xmax) / 2,
	   (size - ymin - ymax) / 2);
...
PUBLIC int
ssv_rna_plot (char *string, char *structure, char *ssfile)
...
/*---------------------------------------------------------------------------*/
PUBLIC int
xrna_plot (char *string, char *structure, char *ssfile)
...
int
PS_dot_plot (char *string, char *wastlfile)
{
...
/*---------------------------------------------------------------------------*/
int
PS_color_dot_plot (char *seq, cpair * pi, char *wastlfile)
{
...
PUBLIC int
simple_xy_coordinates (short *pair_table, float *x, float *y)
{
...
PRIVATE void
loop (int i, int j, short *pair_table)
...
PUBLIC int
PS_dot_plot_list (char *seq, char *wastlfile,
...
int
PS_color_dot_plot_turn (char *seq, cpair * pi, char *wastlfile, int winSize)
{
  /* produce color PostScript dot plot from cpair */
...
int
PS_dot_plot_turn (char *seq, struct plist *pl, char *wastlfile, int winSize)
{
  /* produce color PostScript dot plot from cpair */
...
static FILE *
PS_dot_common (char *seq, char *wastlfile, char *comment, int winsize)
{
