...
static void traverse_loop (struct loop *lp,
...
static void construct_extruded_segment (struct connection *cp,
...
static logical connected_connection (struct connection *cp,
				     struct connection *cpnext);
static int find_ic_middle (int icstart, int icend,
			   struct connection *anchor_connection,
			   struct connection *acp, struct loop *lp);


int
naview_xy_coordinates (short *pair_table, float *X, float *Y)
{
  int i;
...

static void
read_in_bases (short *pair_table)
{
  int i, npairs;
...
*/
{
  ...
  for (i = 0; i <= nbase; i++)
      ...
      if ((mate = bases[i].mate) && !mark[i])
	{
	  ...
	  for (i++, mate--; i < mate && bases[i].mate == mate; i++, mate--)
	    {
	  ...
	  if (debug)
		...
		printf ("\nRegions are:\n");
	      printf ("Region %d is %d-%d and %d-%d with gap of %d.\n",
		      nregion + 1, regions[nregion].start1,
		      regions[nregion].end1, regions[nregion].start2,
		      regions[nregion].end2,
...
*/
{
  ...
  retloop->connections =
    (struct connection **) space (sizeof (struct connection *));
  retloop->depth = 0;
  retloop->number = loop_count;
  retloop->radius = 0.0;
  for (rlp = rlphead; rlp; rlp = rlp->next)
    if (rlp->loopnumber == loop_count)
      retloop->radius = rlp->radius;
  i = ibase;
  do
    {
      if ((mate = bases[i].mate) != 0)
	{
	  ...
	  if (!bases[rp->start1].extracted)
		...
		{
		  fprintf (stderr,
			   "naview: Error detected in construct_loop. i = %d not found in region table.\n",
			   i);
		  exit (FATAL_ERROR);
		}
...
*/
{
  ...
  double dan, dx, dy, rr;
  double cpx, cpy, cpnextx, cpnexty, cnx, cny, rcn, rc, lnx, lny, rl, ac, acn,
    sx, sy, dcp;
  int imaxloop = 0;
  ...
  for (ic = 0; ic < lp->nconnection; ic++)
      ...
      if (fabs (dan - dcp) > pi)
	{
	  if (cp->extruded)
	    {
	      fprintf (stderr,
		       "Warning from traverse_loop. Loop %d has crossed regions\n",
		       lp->number);
	    }
...
static void
determine_radius (struct loop *lp, double lencut)
/*
...
*/
{
  ...
  do
    {
      mindit = 1.0e10;
      for (sumd = 0.0, sumn = 0.0, i = 0; i < lp->nconnection; i++)
	{
	  cp = lp->connections[i];
	  j = i + 1;
	  if (j >= lp->nconnection)
...

static int
find_ic_middle (int icstart, int icend, struct connection *anchor_connection,
		struct connection *acp, struct loop *lp)
/*
...
*/
{
  ...
  if (bases[cp->start].x > anum - 100.0 || bases[cp->end].x > anum - 100.0)
    {
      printf
...

static void
find_center_for_arc (int n, double b, double *hp, double *thetap)
/*
...
#define maxiter 500
  ...
  if (b < 1)
    hlow = 0;			/* otherwise we might fail below (ih) */
  iter = 0;
  do
    {
      h = (hhi + hlow) / 2.0;
	...
	{
	  fprintf (stderr,
		   "Unexpected large magnitude discriminant = %g %g\n", disc,
		   r);
	  exit (FATAL_ERROR);
	}
      theta = acos (disc);
