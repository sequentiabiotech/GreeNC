...
static char rcsid[] UNUSED =
  "$Id: params.c,v 1.7 2004/08/12 12:11:57 ivo Exp $";
...
scale_parameters (void)
{
  ...
  for (; i <= MAXLOOP; i++)
    {
      ...
      p.internal_loop[i] =
	p.internal_loop[30] + (int) (p.lxc * log ((double) (i) / 30.));
    }
  for (i = 0; i < 5; i++)
  ...
  for (i = 0; (i * 7) < strlen (Tetraloops); i++)
    p.TETRA_ENERGY[i] =
      TETRA_ENTH37 - (TETRA_ENTH37 - TETRA_ENERGY37[i]) * tempf;
